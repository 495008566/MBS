...
if __name__ == "__main__":
    # 简单测试 - 5x5网格世界（论文中的第一个示例）
    print("MBS求解器测试 - 5x5网格世界路径规划")
    ...
    R[24] = 1.0  # 右下角作为End（对应论文Figure 4a）
    ...
    solver = MBSolver(
        ...
        noise_level=0.01,  # 噪声级别（论文中的φ）
        enable_noise=True  # 启用噪声，使用近似解
    )
    solver.preprocess(R, P)
    
    # 运行精确解（无噪声）
    solver.set_noise_parameters(enable_noise=False)
    V_exact, W_exact, stats_exact = solver.solve(max_iterations=20)
    print("\n精确解（无噪声）:")
    print(f"MBr迭代次数: {stats_exact['mbr_iterations']}")
    print(f"MBdot操作次数: {stats_exact['mbdot_operations']}")
    
    # 运行近似解（有噪声）
    solver.set_noise_parameters(enable_noise=True)
    V_approx, W_approx, stats_approx = solver.solve(max_iterations=20)
    print("\n近似解（有噪声）:")
    print(f"MBr迭代次数: {stats_approx['mbr_iterations']}")
    print(f"MBdot操作次数: {stats_approx['mbdot_operations']}")
    
    # 打印结果
    print("\n精确解值函数:")
    print(V_exact.reshape(5, 5))
    
    print("\n近似解值函数:")
    print(V_approx.reshape(5, 5))
    
    # 比较两个解的差异
    print("\n解的均方差:")
    mse = np.mean((V_exact - V_approx) ** 2)
    print(f"MSE: {mse:.6f}")
    
    # 打印策略(简化为最佳动作)
    print("\n精确解最佳策略:")
    best_next_states_exact = np.argmax(W_exact, axis=1)
    
    print("\n近似解最佳策略:")
    best_next_states_approx = np.argmax(W_approx, axis=1) 
